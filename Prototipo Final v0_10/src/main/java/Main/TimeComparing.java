...
public class TimeComparing {
	...
	public static void main(String args[]) {
		...
		try {
			...
			Map<ComputeSQLQuery, List<String>> sqlQueriesInfo = new HashMap<ComputeSQLQuery, List<String>>();
			...
			List<String> tableOfTimes = new ArrayList<String>();
			tableOfTimes = CommputeQueriesOnAll(sqlQueriesInfo, connections);
			List<String> tableOfTimesXML = new ArrayList<String>();
			tableOfTimesXML=ComputeXMLQueries(XQueries);

			System.out.println(tableOfTimes);
			System.out.println(tableOfTimesXML);
			// Ending Connections
	...
	public static List<String> CommputeQueriesOnAll(Map<ComputeSQLQuery, List<String>> sqlQueriesInfo,
			...
			for (Connection con : connections) {
				for (String query : sqlQueriesInfo.get(compQuery)) {
					...
					res.add("\n===============\n Query: " + query + "\n Connection: " + con.getCatalog() + "\n Time: "
							+ timeToQuery + "ns " + timeToQuery / 1000000 + "ms\n===============\n");
				}
			}
		}
		return res;
	...
	//TODO Create a file with the results, print only the interest seen as the last one
	public static List<String> ComputeXMLQueries(List<String> queries) throws FileNotFoundException, XQException {
		...
		for (String query : queries) {
			...
			res.add("Time: " + timeToQuery + "ns " + timeToQuery / 1000000 + "ms or "
					+ timeToQuery / 1000000000 + "s" + "\n=====================\n");
		}
		return res;
